...
import { buildVelocityCache, findVelocityThreshold, resolveClip } from '../src/js/utils/velocitySelection.js';
...
describe('resolveClip', () => {
	...
	test('resolves clip for exact velocity thresholds', () => {
		...
		expect(resolveClip(notesData, 60, 0, cache)).toBe('clip-low');
		expect(resolveClip(notesData, 60, 63, cache)).toBe('clip-low');
		expect(resolveClip(notesData, 60, 64, cache)).toBe('clip-high');
		expect(resolveClip(notesData, 60, 127, cache)).toBe('clip-high');
	});

	test('returns null when velocity is below all thresholds', () => {
		const cache = buildVelocityCache(notesData);
		expect(resolveClip(notesData, 60, -1, cache)).toBe(null);
	});

	test('returns null when note has no clips', () => {
		const cache = buildVelocityCache(notesData);
		expect(resolveClip(notesData, 61, 100, cache)).toBe(null);
	});

	test('returns null when velocity threshold points to missing clip', () => {
		const cache = buildVelocityCache(notesData);
		const partial = { 60: { 0: 'clip-low' } };
		expect(resolveClip(partial, 60, 64, cache)).toBe(null);
	});
});
