...
AKVJ uses MIDI notes to trigger clips. Each MIDI channel controls a different layer group or function:
...
- **Channel 5**: Mixer (B&W bitmask for crossfading Groups A & B)
...
- **Channel 13**: Global effects (on entire output)
- **Channel 14**: Reserved (ignored)
- **Channel 15**: Reserved (ignored)
- **Channel 16**: Reserved (ignored)
...
| **Note**     | 0-127 | Which clip to trigger                          |
...
Each channel can play one clip at a time. Sending a new note replaces the current clip.

### Mixer - Layer Group A and Layer Group B Crossfading
...
Set up a MIDI track in your DAW for each AKVJ Channel you want to control. See the **[AKVJ Channel Mapping](#akvj-channel-mapping)** list above for the exact channel numbers and their functions.

> **Folder path note:** Clip source folders use 1-16 (matching your DAW). The build pipeline converts to 0-15 for code output.
...
```
Channel 1 (Layer Group A, Slot 0): Kick clip on beats 1 & 3
  Note 36, Velocity 100

Channel 2 (Layer Group A, Slot 1): Snare clip on beats 2 & 4
  Note 38, Velocity 90
...
- **Note On** = Start clip
- **Note Off** (or Velocity 0) = Stop clip on that channel

For looping clips, hold the note for the desired duration. For one-shot clips, the note length doesn't matter (clip plays to completion).

## Velocity Dynamics

Use velocity to select different clip variants:

```
Soft hit:   Velocity 40  → Subtle clip
Medium hit: Velocity 80  → Normal clip
Hard hit:   Velocity 120 → Intense clip
```

Each velocity can be a completely different clip or a more intense version of the same visual.
...
3. Tempo-synced clips adjust playback speed to match your BPM
...
2. **Check console:** Press F12, look for "JSON for clips loaded"
3. **Check MIDI routing:** Ensure DAW outputs to virtual MIDI port
4. **Check channel numbers:** Use channels 1-16 in your DAW
5. **Check clip exists:** Verify folder at `clips/{ch-1}/{note}/{vel}/`

### Wrong Layer Group responding

Double-check that your MIDI tracks are routed to the correct channels. See the **[AKVJ Channel Mapping](#akvj-channel-mapping)** list at the top of this guide.
...
### Channels (DAW: 1-16 / SOURCE FOLDERS: 1-16)

- **Channel 1**: Layer Group A, Slot 0
- **Channel 2**: Layer Group A, Slot 1
- **Channel 3**: Layer Group A, Slot 2
- **Channel 4**: Layer Group A, Slot 3
- **Channel 5**: Mixer (B&W bitmask for crossfading A & B)
- **Channel 6**: Layer Group B, Slot 0
- **Channel 7**: Layer Group B, Slot 1
- **Channel 8**: Layer Group B, Slot 2
- **Channel 9**: Layer Group B, Slot 3
- **Channel 10**: Mixed output effects (on mixed A & B)
- **Channel 11**: Layer Group C, Slot 0 (overlay layer)
- **Channel 12**: Layer Group C, Slot 1 (overlay layer)
- **Channel 13**: Global effects (on entire output)
- **Channel 14**: Reserved (ignored)
- **Channel 15**: Reserved (ignored)
- **Channel 16**: Reserved (ignored)
...
Each note triggers a different clip.
For effect channels (10, 13):
...
- **0**: Note Off (stop clip)
- **1-127**: Clip variant/intensity
