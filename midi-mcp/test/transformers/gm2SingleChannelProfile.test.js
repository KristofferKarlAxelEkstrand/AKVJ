...
import { transformGm2SingleChannelProfile } from '../../lib/transformers/gm2SingleChannelProfileTransformer.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

describe('GM2 Single Channel Profile (M2-119-UM) Transformer', () => {
	let result;

	beforeAll(async () => {
		const docPath = path.resolve(__dirname, '../../data/m2-119-um-general-midi-2-single-channel-profile.md');
		result = await transformGm2SingleChannelProfile(docPath);
	});

	it('should produce correct metadata', () => {
		expect(result.metadata.title).toBe('General MIDI 2 Single Channel Profile');
		expect(result.metadata.doc_id).toBe('M2-119-UM');
	...
	describe('Normative References', () => {
		...
		it('should parse MA08 (GM2 Function Block Profile)', () => {
			const entry = result.normative_references.find(r => r.ref_id === 'MA08');
			expect(entry).toBeDefined();
			expect(entry.description).toContain('M2-118-UM');
		});
	});
	...
	describe('Definitions', () => {
		...
		it('should parse Controller Message', () => {
			const entry = result.definitions.find(d => d.term === 'Controller Message');
			expect(entry).toBeDefined();
			expect(entry.description).toContain('Control Change');
		});

		it('should parse GM2 Single Channel Profile', () => {
			const entry = result.definitions.find(d => d.term === 'GM2 Single Channel Profile');
			expect(entry).toBeDefined();
			expect(entry.description).toContain('MIDI-CI Profile');
		});

		it('should parse Inquiry', () => {
			const entry = result.definitions.find(d => d.term === 'Inquiry');
			expect(entry).toBeDefined();
			expect(entry.description).toContain('Initiator');
		});
	...
	describe('Conformance Words (Not Relating)', () => {
		...
		it('should parse might as possibility', () => {
	...
	describe('Profile ID', () => {
		...
		it('should parse Byte 2 as GM2 Single Channel Profile Bank', () => {
			const byte2 = result.profile_id.find(b => b.byte === 2);
			expect(byte2.value).toBe('0x20');
			expect(byte2.description).toBe('GM2 Single Channel Profile Bank');
		});

		it('should parse Byte 3 as GM2 Single Channel Profile Number', () => {
			const byte3 = result.profile_id.find(b => b.byte === 3);
			expect(byte3.value).toBe('0x02');
			expect(byte3.description).toBe('GM2 Single Channel Profile Number');
		});

		it('should parse Byte 4 as GM2 Single Channel Profile Version', () => {
			const byte4 = result.profile_id.find(b => b.byte === 4);
			expect(byte4.value).toBe('0x01');
			expect(byte4.description).toBe('GM2 Single Channel Profile Version');
		});

		it('should parse Byte 5 as GM2 Single Channel Profile Level', () => {
			const byte5 = result.profile_id.find(b => b.byte === 5);
			expect(byte5.value).toBe('0x01');
			expect(byte5.description).toBe('GM2 Single Channel Profile Level');
		});
	});
