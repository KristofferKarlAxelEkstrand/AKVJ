...
export async function transformGm2SingleChannelProfile(markdownPath, outDir) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (!line || line.startsWith('## Page') || line.startsWith('# General MIDI 2 Single Channel')) {
			continue;
		}
		...
		if (line.startsWith('Table 4') && line.includes('GM2 Single Channel Profile Id') && !line.match(TOC_DOT_PATTERN)) {
			inProfileId = true;
			continue;
		}
		if (inProfileId) {
			...
			if (line.startsWith('3.3') || line.startsWith('3.2 ')) {
				inProfileId = false;
			}
		}
	}
	...
	if (outDir) {
		const outPath = path.join(outDir, 'gm2-single-channel-profile.json');
