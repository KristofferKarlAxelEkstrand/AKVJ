...
import { validateSetMapping } from './lib/validateMapping.js';
 ...
 *
 * Phases:
 * 1. Validate source clips + set-mapping.json
...
export class Pipeline {
	...
	async run(options = {}) {
		...
		const { errors: mappingErrors } = await validateSetMapping(sourceDir, validClips);
		...
		console.log('  set-mapping.json OK\n');
	...
	async #generate(sourceDir) {
		...
		await fs.copyFile(mappingPath, path.join(this.cacheDir, 'set-mapping.json'));
		console.log('  Copied set-mapping.json');
		console.log();
	}
