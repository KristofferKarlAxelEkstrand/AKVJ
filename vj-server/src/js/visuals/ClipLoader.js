/**
 * ClipLoader - Loads clip assets via set-mapping.json + flat clips.json catalog,
...
class ClipLoader {
	 ...
	 * @param {number} dawChannel - DAW channel 1–16 from set-mapping.json
	 * @param {number|string} note
	 * @param {number|string} velocityThreshold
	 * @param {string} clipId
	 * @param {Object} clipMetadata - Flat catalog entry
	 */
	async #loadMappedClip(dawChannel, note, velocityThreshold, clipId, clipMetadata) {
		...
		try {
			...
			const clip = this.#createClip(image, clipMetadata);
			if (!clip) {
				return null;
			}
			return {
	...
	/**
	 * Set up all clips from mapping + flat catalog URLs.
	 * @param {string} [clipsJsonUrl] - Flat catalog URL (defaults to settings)
	 * @param {string} [setMappingJsonUrl] - Mapping URL (defaults to settings)
	 * @returns {Promise<Object>} Nested clips object keyed by code channel/note/velocity
	 */
	async setupClips(clipsJsonUrl = settings.performance.clipsJsonUrl, setMappingJsonUrl = settings.performance.setMappingJsonUrl) {
		const [clipsCatalog, setMapping] = await Promise.all([this.#loadJson(clipsJsonUrl), this.#loadJson(setMappingJsonUrl)]);

		if (import.meta.env.DEV) {
			console.log('JSON for clips loaded:', clipsCatalog);
			console.log('Set mapping loaded:', setMapping);
		}

		if (!setMapping || !Array.isArray(setMapping)) {
		...
		const loadTasks = this.#buildLoadTasks(setMapping, clipsCatalog);
		const loadResults = await this.#runBatchedLoads(loadTasks);
		return this.#buildClipsObject(loadResults);
	}

	/**
	 * Build load tasks from set-mapping entries and the clip catalog.
	 * @param {Array} setMapping - Mapping entries
	 * @param {Object} clipsCatalog - Flat clip metadata catalog
	 * @returns {Function[]} Array of async load functions
	 */
	#buildLoadTasks(setMapping, clipsCatalog) {
		const loadTasks = [];
		for (const entry of setMapping) {
			const { channel, note, velocity, clipId } = entry ?? {};
			const clipMetadata = clipsCatalog[clipId];
			if (!clipMetadata) {
				console.warn('ClipLoader: mapping references unknown clipId', clipId);
				continue;
			}
			loadTasks.push(() => this.#loadMappedClip(channel, note, velocity, clipId, clipMetadata));
		}
		return loadTasks;
	}
