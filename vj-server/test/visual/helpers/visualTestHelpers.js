...
export function createMaskClip(type) {
	return createMockClip(MASK_DRAW_FNS[type]);
}
...
export function createMockLayerManager({ clipA = null, clipB = null, clipC = null, maskClip = null, maskBitDepth = 1, mixedOutputEffects = [], globalEffects = [] }) {
	const emptyGroup = { hasActiveClips: () => false, getActiveClips: () => [] };
	const makeGroup = clip => (clip ? { hasActiveClips: () => true, getActiveClips: () => [clip] } : emptyGroup);

	return {
		getLayerGroupA: () => makeGroup(clipA),
		getLayerGroupB: () => makeGroup(clipB),
		getLayerGroupC: () => makeGroup(clipC),
		getMaskManager: () => makeMockMaskManager(maskClip, maskBitDepth),
		getEffectsManager: () => makeMockEffectsManager(mixedOutputEffects, globalEffects)
	};
}

function makeMockMaskManager(maskClip, maskBitDepth) {
	return {
...
function makeMockEffectsManager(mixedOutputEffects, globalEffects) {
	return {
