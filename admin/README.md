...
```bash
npm run dev:admin    # UI on :5174, API on :8787
...
The clip validate/optimize/generate pipeline remains in `vj-server/scripts/clips/` (CLI: `npm run clips`). Admin triggers it via `POST /api/pipeline` (`npm run clips -w vj-server`).
