 ...
 * plus clips/set-mapping.json. Verifies before deleting nested trees.
 */
import fs from 'fs/promises';
import path from 'path';
import { createHash } from 'crypto';
import { CLIPS_DIR, SET_MAPPING_PATH, isValidClipId } from './paths.js';
...
async function main() {
	...
	console.log(`Wrote ${SET_MAPPING_PATH}`);
...
async function verifyMigration(inventory) {
	const mappingFile = JSON.parse(await fs.readFile(SET_MAPPING_PATH, 'utf8'));
