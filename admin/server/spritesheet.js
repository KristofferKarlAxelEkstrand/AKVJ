 ...
 * @returns {Promise<{clipId: string, numberOfFrames: number, framesPerRow: number}>}
 */
export async function createClipFromFrames({ clipId, frameBuffers, role }) {
	...
	return { clipId, numberOfFrames: frameBuffers.length, framesPerRow: Math.min(frameBuffers.length, 16) };
}
...
async function writeClipFiles(clipId, role, sheet, numberOfFrames, framesPerRow) {
	...
	const meta = buildClipMeta(role, numberOfFrames, framesPerRow);
	await fs.writeFile(path.join(dir, 'meta.json'), `${JSON.stringify(meta, null, '\t')}\n`);
}

function buildClipMeta(role, numberOfFrames, framesPerRow) {
	const meta = {
		png: 'sprite.png',
		numberOfFrames,
		framesPerRow,
