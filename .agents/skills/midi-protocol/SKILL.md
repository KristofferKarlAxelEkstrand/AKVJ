...
| Error correction | None — lost data is simply lost |
...
USB introduces several ms of random delay per packet. OS scheduling adds further jitter since MIDI is typically low priority. These factors affect MIDI Clock accuracy — AKVJ mitigates this by averaging pulse intervals.

### Byte Format

MIDI uses 8-bit bytes with a strict distinction:

- **Status byte**: Bit 7 set (`1xxxxxxx`, values 128–255) — defines message type and channel
- **Data byte**: Bit 7 clear (`0xxxxxxx`, values 0–127) — carries the payload
...
└─┴─┴─────── Channel number (lower nibble, 0–15)
```
...
| `0xF6`, `0xF8`–`0xFF` | System Real-Time / Tune | 1       |

Invalid status bytes: `0xF4`, `0xF5`, `0xF7`, `0xF9`, `0xFD` — these are reserved/undefined.
...
- `n`: channel (0–15)
- `note`: 0–127 (60 = middle C, 69 = A4 / 440 Hz)
- `velocity`: 1–127 (0 = equivalent to Note Off)
...
| CC# | Name            | Range | Notes                              |
| --- | --------------- | ----- | ---------------------------------- |
| 0   | Bank Select MSB | 0–127 | **Used by AKVJ as BPM fallback**   |
| 1   | Modulation      | 0–127 | Not used by AKVJ                   |
| 7   | Volume          | 0–127 | Not used by AKVJ                   |
| 10  | Pan             | 0–127 | 64 = center                        |
| 64  | Sustain Pedal   | 0/127 | 0–63 = off, 64–127 = on            |
...
CC numbers 120–127 are technically "Channel Mode Messages", not regular Control Change. They affect the entire channel's behavior rather than a continuous controller parameter:
...
14-bit precision: `value = lsb | (msb << 7)`, range 0–16383, center = 8192.
...
RPN (Registered Parameter Number) and NRPN (Non-Registered Parameter Number) extend CC to support >128 parameters with 14-bit resolution (0–16383). They work by selecting a parameter via CC pairs, then setting its value via Data Entry CCs:
...
| (127, 127) | Null — deselect active parameter |
...
Single-byte messages (no channel, no data bytes). These are high-priority timing messages that **can interrupt other messages** in the MIDI stream — the Web MIDI API dispatches them immediately while buffering the interrupted message.
...
- Many devices don't send it — it's entirely optional
- AKVJ correctly ignores it (falls through to the `default` case in the switch)

#### MIDI Clock Timing Details

MIDI Clock sends 24 pulses per quarter note (24 PPQN) — this is fixed by the spec and cannot be changed:
...
- **Manufacturer IDs**: 1-byte (0x01–0x7D) or 3-byte (0x00 + 2 bytes, for IDs > 127)
- **Special IDs**:
...
- **EOX** (`0xF7`): End of Exclusive — terminates the message
...
Running status omits repeated status bytes to save bandwidth. **The Web MIDI API does not allow running status in `MIDIOutput.send()`** — each message must include its full status byte. Incoming running status may or may not be decoded by the browser's MIDI driver.

### Channels: 0–15 vs 1–16

MIDI uses 0–15 internally (in the status byte's lower nibble). DAWs and hardware display channels as 1–16. This is an industry-wide convention.

**In AKVJ**: Source animation folders use 1–16 (matching DAWs). The build pipeline converts to 0–15 for code. Always use 0–15 in code and 1–16 when communicating with users.
...
- **GM1**: 128 instruments (programs 0–127), drum kit on channel 10 (code channel 9)
...
- **Chrome/Chromium only** — required for AKVJ
...
Options:
- `sysex`: boolean (default `false`) — enables System Exclusive messages
- `software`: boolean — enables software MIDI devices
...
- SysEx can be used for firmware updates — browsers inform users of this risk
...
```
MIDI Hardware → Web MIDI API → midi.js (singleton) → AppState (events) → LayerManager/Effects
```
...
2. If message.data is null/undefined or length === 0 → ignore
3. Extract status byte: message.data[0]
4. If status >= 0xF8 → System Real-Time (handle first, return early)
   - 0xF8: dispatchMIDIClock(performance.now())
   - 0xFA: dispatchMIDIStart()
   - 0xFB: dispatchMIDIContinue()
   - 0xFC: dispatchMIDIStop()
5. If message.data.length < 3 → ignore (channel messages need 3 bytes)
6. Extract: command = status >> 4, channel = status & 0xF
7. Extract: note = data[1], velocity = data[2]
8. Switch on command:
   - 9 (Note On): velocity > 0 → dispatchMIDINoteOn; velocity 0 → dispatchMIDINoteOff
...
| `midiStop`                | (no detail)                                     |
| `bpmChanged`              | `{ bpm, source }` — source: `'clock'`/`'cc'`/`'manual'` |
| `bpmSourceChanged`        | `{ source, bpm }` — fired when clock times out, source: `'default'` |
...
2. **MIDI CC (fallback)**: CC number `settings.bpm.controlCC` (default 0) on channel `settings.bpm.controlChannel` (default 0). Maps 0–127 to `settings.bpm.min`–`settings.bpm.max` (10–522 BPM). Only used when clock source is not active.
...
### Strobe Velocity Behavior (Notes 80–95)

- Velocity 0: Off (no effect)
- Velocities 1–9: Full-frame white-out flash (always on)
...
- Duty cycle varies within each bucket: 0.25–0.50 based on the remainder within the 10-velocity bucket (e.g., velocity 10 = 25% duty, velocity 19 = 50% duty)
...
Source folders use 1–16 (DAW convention). Build pipeline converts to 0–15.
...
| `test/utils/midi-fixture.js` | `useFakeMIDIFixture()` — beforeEach/afterEach setup |
| `test/utils/invoke-listeners.js` | `invokeListeners()` — simulate MIDI message events |
| `test/utils/wait-for-event.js` | `waitForEvent()` — async wait for AppState events |
...
- `navigator.requestMIDIAccess` will fail — always use fake MIDI utilities in tests
...
### Channel Numbers
- **Code uses 0–15**, DAWs display 1–16. Always clarify which convention you're using.
- Source animation folders use 1–16; the build pipeline converts to 0–15.
...
- **Active Sensing** (`0xFE`) helps detect dead connections — receiver turns off all voices after 300ms timeout
- **All Notes Off** (CC 123) can be sent as a panic button
- **All Sound Off** (CC 120) is more aggressive — immediately silences
...
- Single-byte messages — no channel, no data bytes.

### Timing
- Use `performance.now()` for all timing, never `Date.now()`.
- AKVJ passes `performance.now()` to `dispatchMIDIClock()`, not `message.timeStamp` — this ensures timing consistency with the render loop.
...
### Web MIDI API Quirks
- No running status in `MIDIOutput.send()` — always send complete messages
- `addEventListener` vs `onmidimessage` — AKVJ supports both via feature detection
- Port auto-opens when `onmidimessage` is set
- Some systems poll for new devices infrequently — don't rely solely on `statechange` events

### Performance
- MIDI message handling must be < 20ms latency (input to visual output)
- Never block the render loop with MIDI processing
- BPM calculation is O(n) where n = PPQN (24) — negligible cost
...
- Wrap in try-catch — `requestMIDIAccess()` can reject (permission denied)

## Existing Documentation

For deeper reference, see these project docs:

- `docs/midi-protocol-guide.md` — Full MIDI 1.0 protocol reference with code examples
- `docs/how-to-program-midi.md` — DAW setup guide and AKVJ channel mapping
- `docs/midi-controller-reference.md` — All 128 CC numbers with descriptions
- `docs/usb-midi-and-midi2-reference.md` — USB-MIDI packet format and MIDI 2.0 overview
...
The Web MIDI API does not yet support MIDI 2.0 (as of 2025). AKVJ targets MIDI 1.0 only. If MIDI 2.0 support is added in the future, design internal values as normalized (0–1) for easy scaling between bit depths.
