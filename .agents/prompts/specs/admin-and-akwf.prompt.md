# Monorepo Refactor & Admin Dashboard Specification

## Motivation

Clips are currently identified solely by their position in a `clips/{channel}/{note}/{velocity}/` folder. Renaming, reusing, or reorganizing a clip means moving files and re-deriving the MIDI mapping by hand — there's no way to build or edit a set visually, and no concept of a clip's identity independent of where it happens to sit in the MIDI grid. This spec decouples clip identity from MIDI position (a flat clip bucket + an explicit mapping file) and gives that new workflow a visual Admin dashboard, while keeping the live-performance engine (`vj-server`) exactly as fast and constrained as it is today.
...
- `/vj-server/`: The existing Vite application (the core VJ engine). Stripped of all build scripts and clip-generation logic. Its sole purpose is to consume MIDI, read `set-mapping.json`, and render at 60fps. **Vanilla JS, ES6+ modules — same architecture rules as today, unchanged.**
- `/admin/`: A new Vite application for managing the VJ set, creating clips, and building sprite sheets. **Also vanilla JS + vanilla CSS — no framework, no Tailwind.** This repo has one architecture rule enforced everywhere: no frameworks. A second package with a different stack would mean two build philosophies to maintain and would need its own carved-out exception to `AGENTS.md`; staying vanilla avoids that entirely.
- `/clips/`: A shared, top-level "bucket" directory where all raw clip data and metadata lives — used by both `/vj-server/` (read-only, at runtime) and `/admin/` (read/write, at authoring time).

Root `package.json` gains a `"workspaces": ["vj-server", "admin"]` field. Each package keeps its own `package.json`, `vite.config.js`, and `node_modules` (hoisted by npm where possible); one `npm install` at the root installs both.
...
## 3. The Admin Dashboard Responsibilities

The Admin UI (vanilla JS + vanilla CSS) and its accompanying local backend take over all set preparation logic. The backend is a minimal Node `http`/`fs` server — no Express dependency needed for local file read/write plus `sharp` calls:
...
1. Move the existing codebase (except `.agents`, `clips`, `docs`, and `.git`) into a new `/vj-server/` directory.
2. Add `"workspaces": ["vj-server", "admin"]` to the root `package.json`.
3. Update `vite.config.js`'s `root`, `build.outDir`, and `rollupOptions.input` (all currently `__dirname`-relative, assuming a repo-root single package) to resolve correctly from inside `vj-server/`.
...
5. Update `.devcontainer/devcontainer.json`: add `forwardPorts` entries for the Admin dev server and its backend (neither exists today — only 5173/4173 are forwarded), and reassess the 2 CPU / 4 GB resource minimum against running two Vite dev servers plus a Node backend plus `sharp` processing concurrently.
6. Update `.github/workflows/ci.yml` for per-workspace steps (`npm ci` at root, then lint/test/build for each of `vj-server`/`admin`), and fix the existing hardcoded `src/public/clips` output-verification step to match wherever the pipeline output lands post-refactor.
7. Verify that `npm install && npm run dev` (from repo root, via workspaces) still starts `vj-server` and renders correctly.

**Phase 2: Admin Scaffolding**

1. Initialize a new vanilla-JS Vite project in `/admin/` — same architecture rules as `vj-server` (no framework, no Tailwind; plain CSS).
...
1. Build the Admin "Clip Library" UI to view all clips in the bucket.
2. Build the Admin "Uploader" to ingest new images and write them to the bucket.
3. Build the Admin "Mapping" UI to modify and save `set-mapping.json`.
