import { createAppState } from '../src/js/core/AppState.js';
...
describe('AppState', () => {
	...
	test('reset() prevents pending clock timeout from firing stale BPM source change', () => {
		vi.useFakeTimers();

		// Send enough clock pulses to set BPM source to clock
		const baseTime = 1000;
		for (let i = 0; i < 8; i++) {
			appState.dispatchMIDIClock(baseTime + i * 50);
		}
		expect(appState.bpmSource).toBe('clock');

		// Send one more pulse to schedule the clock timeout
		appState.dispatchMIDIClock(baseTime + 8 * 50);

		// Reset before the timeout fires
		appState.reset();
		expect(appState.bpmSource).toBe('default');

		// Advance past the clock timeout duration
		vi.advanceTimersByTime(600);

		// BPM source should remain default — the stale timeout was discarded
		expect(appState.bpmSource).toBe('default');
